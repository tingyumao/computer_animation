...
def guess_fosssim():
    guess = "./oracle/FOSSSimOracleT1M2"
...
def main():
    ...
    scene_outputbin = scene_fullpath.replace(".xml", ".bin")
    fosssim_invocation = "{0} -s {1} -i {2} -d 0 {3}".format(fosssim, scene_fullpath, scene_outputbin, ' '.join(args.flags))
